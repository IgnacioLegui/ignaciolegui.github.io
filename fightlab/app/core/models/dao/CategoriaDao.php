...
final class CategoriaDao extends BaseDao implements InterfaceDao{
    ...
    public function save(array $data): void{
        $sql = "INSERT INTO {$this->table} VALUES(DEFAULT, '{$data['nombre']}')";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        if($stmt->rowCount() === 0){
    ...
    public function update(array $data): void{
        $sql = "UPDATE {$this->table} SET nombre = '{$data['nombre']}' WHERE id = {$data['id']}";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        if($stmt->rowCount() === 0){
            throw new \Exception("No se pudo actualizar el registro con ID {$data['id']}");
        }
    }
